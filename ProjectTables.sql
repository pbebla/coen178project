...
create table Log(repairjobId int primary key, car_license_no varchar(5), time_in timestamp, time_out timestamp, emp_id int, laborhrs decimal(*,2), bill decimal(*,2), foreign key(car_license_no) references Car(car_license_no), foreign key(emp_id) references Mechanic(emp_id));
